...
The result is `O(n)` transfers in the typical case rather than the naive `O(n²)`.
...
- **Node.js 20+** and npm
- A **Postgres database**. The defaults assume [Neon](https://neon.tech) (the HTTP driver is wired up via `@neondatabase/serverless`), but any Postgres reachable via a connection string will work.
...
The app throws at startup if `JWT_SECRET` is missing or shorter than 32 chars.
